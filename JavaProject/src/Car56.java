...
public class Car56 extends Car implements SignalListener {
    ...
    public void updatePos() {
        ...
        {
        	//System.out.println("helloooooo00000000000000000000");
    	...
    	{	
    		float a = (((this.getSpeed()*this.getSpeed()))/(2*distToNextInter()));
    		accelerate(-a,10);
    	}
        ...
        if(this.getDir() == this.getRoad().getDir())
        {
        	...
        	if(trafficC == null)
        	{
        	...
        	else if(trafficC != null )
	     	{
        		...
        		if(signal == 0 && distToNextInter() <= 5)
        		{
	        		accelerate(-1000,10);
	        		System.out.println("3 if");
	        	}
        	
	        	else if(signal == 1 /*&& flag == 1*/)
	        	{
	        		synchronized (trafficC)
	        		{
    ...
    protected void turn(int dir){
        ...
        turned = 0;
        if(this.getDir() == this.getRoad().getDir()){
        	if(Math.abs(this.getPos().y - this.getRoad().getEndIntersection().getCoords().y) <= 30  && Math.abs(this.getPos().x - this.getRoad().getEndIntersection().getCoords().x) <= 30)
            {
        		dir = Math.abs(rand.nextInt())%4;
        	...
        	if(Math.abs(this.getPos().y - this.getRoad().getStartIntersection().getCoords().y) <= 30  && Math.abs(this.getPos().x - this.getRoad().getStartIntersection().getCoords().x) <= 30)
            {
        		dir = Math.abs(rand.nextInt())%4;
